...
export default function CarouselSection({ projects, showMetadata = true }) {
  ...
  useEffect(() => {
    ...
    async function mountCarousel() {
      ...
      const engine = createCarousel(mountRef.current, { projects, cursorElement: cursorRef.current, onActiveChange: setActive, onFocusChange: setFocused, onEntryDone: setEntryDone, onProjectOpen: (index) => router.push(`/work/${projects[index].slug}`) });
  ...
  return <section ref={mountRef} className={`carousel-shell ${entryDone ? "entry-done" : "is-entering"}`} aria-label="Selected video work">
    ...
    {focused && project && <div className="focus-project"><div><span>{project.category?.name || "Selected work"} · {project.year || ""}</span><h2>{project.title}</h2><p>{project.description || project.shortDescription}</p></div><ProjectMedia project={project} context="focus-player" autoPlay onClose={() => engineRef.current?.closeFocus()} /></div>}
  </section>;
}
