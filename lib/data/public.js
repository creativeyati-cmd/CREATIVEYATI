...
function textureUrl(row) {
  ...
  return variants.desktop || variants.tablet || row.cover_image_url || row.custom_poster_url || row.video_thumbnail_url || row.youtube_thumbnail_url || "/img1.png";
}
...
const mapVideo = (row) => ({
  ...
  coverAspectRatio: Number(row.cover_aspect_ratio) || 16 / 9,
  ...
  coverAlt: row.cover_alt || `${row.title || "Project"} cover`,
  videoProvider: row.video_provider || (row.youtube_video_id ? "youtube" : ""),
  videoUrl: row.video_url || row.youtube_url || "",
  videoAssetId: row.video_asset_id || row.youtube_video_id || "",
  videoEmbedUrl: row.video_embed_url || row.youtube_embed_url || "",
  videoThumbnailUrl: row.video_thumbnail_url || row.youtube_thumbnail_url || "",
  youtubeThumbnailUrl: row.youtube_thumbnail_url || "", youtubeUrl: row.youtube_url || "", youtubeVideoId: row.youtube_video_id || "", orientation: row.orientation || "landscape",
