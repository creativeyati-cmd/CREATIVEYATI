...
import { normaliseProfileImageUrl } from "@/lib/profile-image";
...
export async function getSiteContent() {
  ...
  const value = { ...siteDefaults, ...(data?.value || {}), ...(legacyContact?.value || {}), ...(contact?.value || {}) };
  return { ...value, profileImage: normaliseProfileImageUrl(value.profileImage) };
}
