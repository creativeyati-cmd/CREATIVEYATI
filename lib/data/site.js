...
export const siteDefaults = {
  creatorName: "Frame / Motion", profileImage: "", heroHeading: "I turn brand ideas into stories people want to watch.", highlightWord: "stories",
  ...
  professionalTitle: "Director · Video Creator · Visual Storyteller", availability: "Available for select collaborations",
  ...
  contactHeading: "Have a story in mind?", contactCopy: "Tell me what you are making and where it needs to go.", publicEmail: "", phone: "", instagramUrl: "", youtubeUrl: "", footerText: "",
  accentColor: "#a3c900", lightBackground: "#f2f0eb", darkBackground: "#121212",
};

export async function getSiteContent() {
  const supabase = await createSupabaseServerClient();
  if (!supabase) return siteDefaults;
  const { data } = await supabase.from("site_content").select("value").eq("key", "site").maybeSingle();
  return { ...siteDefaults, ...(data?.value || {}) };
}
