...
export function createMobileLiquidLens(mount, imageElement) {
  ...
  function refreshLayout() {
    ...
    const left = (imageRect.left - mountRect.left) / width;
    const bottom = 1 - (imageRect.bottom - mountRect.top) / height;
    const posterWidth = imageRect.width / width;
    const posterHeight = imageRect.height / height;
    uniforms.uPosterRect.value.set(left, bottom, posterWidth, posterHeight);

    const lensX = clamp(left + posterWidth * 0.88, 0.12, 1.05);
    const lensY = clamp(bottom + posterHeight * 0.38, 0.12, 0.88);
    uniforms.uCenter.value.set(lensX, lensY);
    const lensRadius = clamp(posterHeight * 0.46, 0.22, 0.46);
    uniforms.uLensSize.value.set(lensRadius * 0.78, lensRadius);
  }
