...
export function createCarouselGui(carousel) {
  const { lensUniforms, refreshLayout, replayEntry, setInteraction, setMotion } = carousel;
  ...
  scrollFolder.add(CONFIG, "WHEEL", 0.1, 4, 0.05).name("wheel sensitivity");
  ...
  const motionControls = { ...MOTION };
  const motionFolder = gui.addFolder("Continuous motion");
  motionFolder.add(motionControls, "enabled").name("enabled").onChange((enabled) => setMotion({ enabled }));
  motionFolder.add(motionControls, "direction", ["left", "right"]).name("direction").onChange((direction) => setMotion({ direction }));
  motionFolder.add(motionControls, "desktopSpeed", 10, 60, 1).name("desktop px/s").onChange((desktopSpeed) => setMotion({ desktopSpeed }));
  motionFolder.add(motionControls, "mobileSpeed", 10, 40, 1).name("mobile px/s").onChange((mobileSpeed) => setMotion({ mobileSpeed }));
  motionFolder.add(motionControls, "resumeDelay", 0, 6000, 100).name("resume delay").onChange((resumeDelay) => setMotion({ resumeDelay }));
  motionFolder.add(motionControls, "disableForReducedMotion").name("honour reduced motion").onChange((disableForReducedMotion) => setMotion({ disableForReducedMotion }));
  ...
  entryFolder.add(ENTRY, "startScale", 0.5, 1, 0.01).name("start scale");
  entryFolder.add(ENTRY, "scaleOvershoot", 1, 1.08, 0.001).name("scale overshoot");
  entryFolder.add(ENTRY, "positionOvershoot", 1, 1.08, 0.001).name("rise overshoot");
  entryFolder.add(ENTRY, "riseDuration", 0.3, 3, 0.05).name("rise dur (s)");
  entryFolder.add(ENTRY, "settleDuration", 0.1, 0.8, 0.01).name("settle dur (s)");
  ...
  entryFolder.add(ENTRY, "settleEase", GSAP_EASES).name("settle ease");
