...
import { CONFIG, INTERACT, LENS, FOCUS, ENTRY, AUTOPLAY } from "./config";
...
export function createCarousel(mount, callbacks = {}) {
  ...
  const REPEATS = sources.length === 1 ? 5 : 4;
  const pool = [];
  ...
  let autoplayMoving = false;
  let autoplayNextAt = Infinity;
  let autoplayResumeAt = 0;
  let carouselVisible = true;
  let navigationOpen = false;
  let interactiveFocused = false;

  function deferAutoplay(delay = AUTOPLAY.resumeDelay) {
    autoplayMoving = false;
    autoplayResumeAt = performance.now() + delay;
    autoplayNextAt = Infinity;
  }

  function scheduleAutoplay(delay = W <= 620 ? AUTOPLAY.mobileRest : AUTOPLAY.rest) {
    autoplayNextAt = performance.now() + delay;
  }

  function autoplayPaused() {
    return !AUTOPLAY.enabled || reducedMotion.matches || !autoplayReady || dragging || (pointerInside && lastPointerType === "mouse") || focusState.active || navigationOpen || document.hidden || !carouselVisible || interactiveFocused;
  }
  ...
  function onEnter(e) {
    pointerInside = true;
    lastPointerType = e.pointerType || "mouse";
    if (lastPointerType === "mouse") deferAutoplay();
  }
  function onLeave() {
    pointerInside = false;
    setHover(false);
  ...
  function playEntry() {
    ...
    tl.call(
      () => {
        ...
        onEntryDone(true);
        autoplayReady = true;
        scheduleAutoplay(AUTOPLAY.initialDelay);
  ...
  function onNavigationChange(event) {
    ...
    if (navigationOpen) deferAutoplay();
    else deferAutoplay();
  }
  function onVisibilityChange() {
    if (document.hidden) deferAutoplay();
    else deferAutoplay();
  }
  function onFocusIn(event) {
    if (event.target !== el) interactiveFocused = true;
    deferAutoplay();
  }
  function onFocusOut() {
    queueMicrotask(() => {
      interactiveFocused = mount.contains(document.activeElement) && document.activeElement !== el;
      if (!interactiveFocused) deferAutoplay();
    });
  }
  ...
  const viewportObserver = new IntersectionObserver(([entry]) => {
    ...
    if (!carouselVisible) deferAutoplay();
    else deferAutoplay();
  }, { threshold: 0.12 });
  ...
  function tick() {
    ...
    if (!autoplayPaused() && now >= autoplayResumeAt) {
      if (!autoplayMoving && now >= autoplayNextAt) {
        velocity = 0;
        pendingFocus = null;
        ...
        autoplayMoving = true;
        autoplayNextAt = Infinity;
      } else if (autoplayMoving && Math.abs(target - scroll) < 0.7) {
        autoplayMoving = false;
        scheduleAutoplay();
      }
    }
  ...
  if (ENTRY.enabled) playEntry();
    ...
    autoplayReady = !reducedMotion.matches;
    onEntryDone(true);
    scheduleAutoplay(AUTOPLAY.initialDelay);
  }
