...
import { CONFIG, INTERACT, LENS, FOCUS, ENTRY } from "./config";
...
function measureCarousel(width, height) {
  // This is the only responsive scale in the scene. The frame is measured
  // first; the gap, lens and entry travel are all derived from that frame.
  const responsiveScale = scaleForViewport(width);
  const frameHeight = CONFIG.PANEL_H * responsiveScale;
  const frameWidth = frameHeight * COVER_ASPECT;
  const lensWidth = frameWidth * LENS.sizeX;
  const lensHeight = frameHeight * LENS.sizeY;
  const desktopLensWidth = CONFIG.PANEL_H * COVER_ASPECT * LENS.sizeX;
  return {
    responsiveScale,
    frameWidth,
    frameHeight,
    gap: CONFIG.GAP * responsiveScale,
    lensWidth,
    lensHeight,
    lensScaleX: (lensWidth * 0.5) / Math.max(height, 1),
    lensScaleY: (lensHeight * 0.5) / Math.max(height, 1),
    ringWidth: Math.min(0.04, Math.max(0.004, (LENS.ringWidth * desktopLensWidth) / Math.max(lensWidth, 1))),
    rimLineWidth: Math.min(0.025, Math.max(0.0015, (LENS.rimLineWidth * desktopLensWidth) / Math.max(lensWidth, 1))),
  };
}
...
export function createCarousel(mount, callbacks = {}) {
  ...
  let layoutMetrics = measureCarousel(W, H);
  let finalLensScale = {
    x: layoutMetrics.lensScaleX,
    y: layoutMetrics.lensScaleY,
  };
  ...
  function refreshLayoutMetrics() {
    layoutMetrics = measureCarousel(W, H);
    finalLensScale = {
      x: layoutMetrics.lensScaleX,
      y: layoutMetrics.lensScaleY,
    };
    mount.style.setProperty("--carousel-frame-width", `${layoutMetrics.frameWidth * ACTIVE_SCALE}px`);
    mount.style.setProperty("--carousel-frame-height", `${layoutMetrics.frameHeight * ACTIVE_SCALE}px`);
  }
  ...
  function panelHeight() { return layoutMetrics.frameHeight; }

  function panelGap() { return layoutMetrics.gap; }
  ...
  // ---- mesh pool ----
  // A single published project keeps one centered mesh plus decorative
  // left/right copies. It remains logically 01/01 and cannot be navigated.
  const REPEATS = hasMultipleProjects ? 4 : 3;
  const pool = [];
  ...
  const lensUniforms = {
    ...
    uSizeX: { value: finalLensScale.x },
    uSizeY: { value: finalLensScale.y },
    ...
    uRingWidth: { value: layoutMetrics.ringWidth },
    ...
    uRimLineWidth: { value: layoutMetrics.rimLineWidth },
  ...
  const focusState = {
    ...
    lensEntryScale: ENTRY.enabled ? entryStartScale() : 1,
    anim: null,
  };
  ...
  const finalFrameTargets = new Array(REPEATS * sources.length);
  let entryAnim = null;
  ...
  function layout() {
    ...
    pool.forEach((p, poolIdx) => {
      ...
      const activeBoost = 1 + (ACTIVE_SCALE - 1) * Math.max(0, 1 - Math.abs(centerX) / Math.max(baseW, 1));
      ...
      // Snapshot the exact final position and scale before applying entry
      // progress. The timeline always settles back to these measured values.
      finalFrameTargets[poolIdx] = {
        finalPosition: { x: centerX, y },
        finalScaleX: drawW,
        finalScaleY: drawH,
      };
      ...
      if (entryActive || entrySettled) {
        ...
        const finalTarget = finalFrameTargets[poolIdx];

        const startScale = entryStartScale();
        const curScale = startScale + (1 - startScale) * g;
        finalH = finalTarget.finalScaleY * curScale;
        finalW = finalTarget.finalScaleX * curScale;
        ...
        const below = -Math.max(H * 0.58, layoutMetrics.frameHeight * ENTRY.fromBelow);
        finalY = below + (finalTarget.finalPosition.y - below) * pe;
      }
  ...
  function updateCursor() {
    ...
    if (dragging && hasMultipleProjects) return setCursor("grabbing");
    if (!hoverPanel) return setCursor(""); // off the carousel, plain arrow
    if (INTERACT.drag && hasMultipleProjects) return setCursor("grab");
  ...
  function setView(on) {
    ...
    if (dragging && hasMultipleProjects) on = false;
    if (on === overPanel) {
      updateCursor();
      return;
    }
  ...
  function onPointerUp(e) {
    ...
    velocity = hasMultipleProjects &&
      performance.now() - dragMoveT <= INTERACT.FLICK_IDLE_MS ? dragVel : 0;
    dragVel = 0;
    lastInput = performance.now();
  ...
  function playEntry() {
    ...
    // The glass wakes just after the centered frame, grows against the stored
    // final lens dimensions, then settles to exactly 1 (never cumulatively).
    const lensSettleAt = 0.12 + ENTRY.lensBloom;
    tl.to(
      focusState,
      { lensFx: 1, lensEntryScale: entryScaleOvershoot(), duration: ENTRY.lensBloom, ease: ENTRY.lensBloomEase },
      0.12,
    );
    tl.to(focusState, { lensEntryScale: 1, duration: ENTRY.settleDuration, ease: ENTRY.settleEase }, lensSettleAt);
    entryEnd = Math.max(entryEnd, lensSettleAt + ENTRY.settleDuration);
  ...
  let raf;
  function tick() {
    if (hasMultipleProjects && !dragging) {
    ...
    lensUniforms.uSizeX.value = finalLensScale.x * focusState.lensEntryScale;
    lensUniforms.uSizeY.value = finalLensScale.y * focusState.lensEntryScale;
    lensUniforms.uRingWidth.value = layoutMetrics.ringWidth;
    lensUniforms.uRimLineWidth.value = layoutMetrics.rimLineWidth;
  ...
  const themeObserver = new MutationObserver(syncClearColor);
