...
export function createCarousel(mount, callbacks = {}) {
  ...
  // ---- mesh pool ----
  // A single published project gets one mesh. Infinite repetitions only exist
  // when there is actually something to navigate between.
  const REPEATS = hasMultipleProjects ? 4 : 1;
  const pool = [];
