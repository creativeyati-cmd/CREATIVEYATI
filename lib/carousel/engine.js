...
export function createCarousel(mount, callbacks = {}) {
  ...
  const sources = projects.map((img) => {
    const s = {
      tex: null,
      aspect: img.aspect || img.aspectRatio || 1,
      locked: img.aspect != null || img.aspectRatio != null,
    };
  ...
  // Responsive panel measurements are based on the real carousel viewport.
  // Landscape work is width-led and cinematic; portrait work is height-led.
  function panelHeight(srcIndex) {
    const aspect = Math.max(sources[srcIndex]?.aspect || 1, 0.1);
    if (aspect < 1) return Math.min(H * 0.69, (W * 0.34) / aspect);
    return Math.min(H * 0.66, (W * 0.56) / aspect);
  }

  function panelGap() { return Math.max(10, Math.min(18, W * 0.009)); }
  ...
  const REPEATS = sources.length === 1 ? 1 : 4;
  const pool = [];
  ...
  function layout() {
    ...
    pool.forEach((p, poolIdx) => {
      ...
      if (entryActive || entrySettled) {
        ...
        const curScale = ENTRY.startScale + (1 - ENTRY.startScale) * g;
        ...
        const slotH = (s) => {
          ...
          return panelHeight(s) * (ENTRY.startScale + (1 - ENTRY.startScale) * gg);
        };
        let off = 0;
        if (di > 0) {
          for (let k = 0; k < di; k++) {
  ...
  function playEntry() {
    ...
    const overshootProgress = (ENTRY.scaleOvershoot - ENTRY.startScale) / Math.max(1 - ENTRY.startScale, 0.001);
    ...
    visible.forEach(({ idx }, rank) => {
      ...
      tl.to(pEntry, { [idx]: ENTRY.positionOvershoot, duration: ENTRY.riseDuration, ease: ENTRY.riseEase }, at);
