...
import { CONFIG, INTERACT, LENS, FOCUS, ENTRY, MOTION } from "./config";

const DESIGN_WIDTH = 1920;
const DESIGN_STAGE_HEIGHT = 800;
const DESIGN_PANEL_HEIGHT = 560;
const DESIGN_GAP = 12;
const COVER_ASPECT = 16 / 9;

function scaleForViewport(width) {
  if (width <= 1024) return Math.min(0.6, 0.245 + Math.max(0, width - 320) * (0.355 / 704));
  return Math.min(1, 0.6 + (width - 1024) * (0.4 / 896));
}
...
export function createCarousel(mount, callbacks = {}) {
  ...
  const motionConfig = {
    enabled: motion.enabled ?? MOTION.enabled,
    direction: motion.direction === "right" ? "right" : "left",
    desktopSpeed: boundedNumber(motion.desktopSpeed, MOTION.desktopSpeed, 10, 60),
    mobileSpeed: boundedNumber(motion.mobileSpeed, MOTION.mobileSpeed, 10, 40),
    resumeDelay: boundedNumber(motion.resumeDelay, MOTION.resumeDelay, 0, 6000),
    disableForReducedMotion: motion.disableForReducedMotion ?? MOTION.disableForReducedMotion,
  };

  const responsiveScale = () => scaleForViewport(W);
  const renderPixelRatio = () => Math.min(window.devicePixelRatio || 1, W <= 1024 ? 1.5 : 2);
  const entryStartScale = () => W <= 1024 ? 0.76 : ENTRY.startScale;
  const entryScaleOvershoot = () => W <= 1024 ? 1.03 : ENTRY.scaleOvershoot;
  const entryPositionOvershoot = () => W <= 1024 ? 1.03 : ENTRY.positionOvershoot;

  // ---- renderer / scene / camera (orthographic, 1 unit = 1 px) ----
  const renderer = new THREE.WebGLRenderer({ antialias: true, alpha: true, powerPreference: W <= 1024 ? "low-power" : "high-performance" });
  renderer.setPixelRatio(renderPixelRatio());
  renderer.setSize(W, H);
  function syncClearColor() {
    const color = getComputedStyle(mount).backgroundColor || "#ffffff";
    renderer.setClearColor(new THREE.Color().setStyle(color), 1);
  }
  syncClearColor();
  renderer.domElement.className = "carousel-canvas";
  ...
  function applyCoverCrop(source, texture) {
    if (!texture.image?.width || !texture.image?.height) return;
    const imageAspect = texture.image.width / texture.image.height;
    const focalX = Math.min(100, Math.max(0, Number(source.focalX) || 50)) / 100;
    const focalY = Math.min(100, Math.max(0, Number(source.focalY) || 50)) / 100;
    texture.repeat.set(1, 1);
    texture.offset.set(0, 0);
    if (imageAspect > COVER_ASPECT) {
      texture.repeat.x = COVER_ASPECT / imageAspect;
      texture.offset.x = (1 - texture.repeat.x) * focalX;
    } else if (imageAspect < COVER_ASPECT) {
      texture.repeat.y = imageAspect / COVER_ASPECT;
      texture.offset.y = (1 - texture.repeat.y) * (1 - focalY);
    }
  ...
  const sources = projects.map((img) => {
    const s = {
      tex: null,
      aspect: COVER_ASPECT,
      focalX: img.coverFocalX ?? img.cover_focal_x ?? 50,
      focalY: img.coverFocalY ?? img.cover_focal_y ?? 50,
    };
    loader.load(img.posterUrl || img.src, (tex) => {
  ...
  // Every viewport looks through a differently sized window at one stable
  // desktop composition. Covers, gaps, entry travel and glass share this scale.
  function panelHeight() { return DESIGN_PANEL_HEIGHT * responsiveScale(); }

  function panelGap() { return DESIGN_GAP * responsiveScale(); }

  function slotWidth(srcIndex) {
    return sources[srcIndex].aspect * panelHeight(srcIndex) + panelGap();
  }
  ...
  function centerForIndex(idx) {
    ...
    const s = ((idx % N) + N) % N;
    return offsets[s] + slotWidth(s) / 2 - panelGap() / 2 + loop * totalWidth;
  }
  ...
  function nearestIndex(value) {
    ...
    for (let i = 0; i < N; i++) {
      const center = offsets[i] + slotWidth(i) / 2 - panelGap() / 2;
  ...
  // ---- mesh pool ----
  // Keep only the neighbouring loop copies needed to cover the viewport.
  // A one-project portfolio renders exactly one mesh and never duplicates it.
  const REPEATS = sources.length === 1 ? 1 : 3;
  const pool = [];
  ...
  let velocity = 0; // flick momentum after a drag release, in CSS px / second
  let prevScroll = 0;
  ...
  let lastInput = performance.now(); // timestamp of last wheel/drag input
  let snapped = false; // have we already snapped since the last input?
  const reducedMotion = window.matchMedia("(prefers-reduced-motion: reduce)");
  let motionReady = false;
  let automaticVelocity = 0;
  let motionResumeAt = Infinity;
  let carouselVisible = true;
  let navigationOpen = false;
  let interactiveFocused = false;

  function deferMotion(delay = motionConfig.resumeDelay) {
    automaticVelocity = 0;
    motionResumeAt = performance.now() + delay;
  }

  function freezeMotion(delay = motionConfig.resumeDelay) {
    automaticVelocity = 0;
    velocity = 0;
    target = scroll;
    motionResumeAt = performance.now() + delay;
  ...
  function continuousMotionEnabled() {
    return motionConfig.enabled && sources.length > 1 && !(motionConfig.disableForReducedMotion && reducedMotion.matches);
  }

  function automaticSpeed() {
    const magnitude = W <= 1024 ? motionConfig.mobileSpeed : motionConfig.desktopSpeed;
    return motionConfig.direction === "right" ? -magnitude : magnitude;
  }

  function motionPaused() {
    return !continuousMotionEnabled() || !motionReady || dragging || pendingFocus || focusState.active || navigationOpen || document.hidden || !carouselVisible || interactiveFocused;
  }
  ...
  const lensUniforms = {
    ...
    uSizeX: { value: (LENS.sizeX * DESIGN_STAGE_HEIGHT * responsiveScale()) / Math.max(H, 1) },
    uSizeY: { value: (LENS.sizeY * DESIGN_STAGE_HEIGHT * responsiveScale()) / Math.max(H, 1) },
    ...
    uSamples: { value: W <= 1024 ? Math.min(LENS.samples, 10) : LENS.samples },
  };
  ...
  // ---- entry state ----
  // pEntry controls the upward rise; growArr controls scale from startScale.
  ...
  function layout() {
    ...
    const half = W / 2;
    const buffer = Math.max(...sources.map((_, index) => panelHeight(index)), H * 0.5);
    pool.forEach((p, poolIdx) => {
      ...
      const slotCenterInLoop = offsets[i] + slotWidth(i) / 2 - panelGap() / 2;
      ...
      const shrink = 1 - 0.18 * scrollEnergy;
      const baseH = panelHeight(i);
      const baseW = src.aspect * baseH;
      const activeBoost = 1 + 0.055 * Math.max(0, 1 - Math.abs(centerX) / Math.max(baseW, 1));
      const h = baseH * shrink * activeBoost;
      const wPx = baseW * shrink * activeBoost;
      ...
      // Entry: rise from below while scaling from the configured start ratio.
      let finalX = centerX;
      let finalY = y;
      let finalW = drawW;
      let finalH = drawH;
      if (entryActive || entrySettled) {
        ...
        const startScale = entryStartScale();
        const curScale = startScale + (1 - startScale) * g;
        const curH = drawH * curScale;
        finalH = curH;
        ...
        const slotH = (s) => {
          ...
          return panelHeight(s) * (startScale + (1 - startScale) * gg);
        };
        let off = N === 1 ? (rep - midRep) * (sources[0].aspect * slotH(0) + panelGap()) : 0;
        if (N > 1 && di > 0) {
          for (let k = 0; k < di; k++) {
        ...
        const below = -Math.max(H * 0.58, DESIGN_PANEL_HEIGHT * 1.18 * responsiveScale());
  ...
  function panelAtPointer(px, py) {
    // Pointer events report viewport coordinates while panelRects are local to
    // the canvas. Accounting for the canvas offset keeps touch hit-testing
    // accurate after the header and responsive top spacing move the carousel.
  ...
  // Touch hardening: own the gesture outright so the browser never steals it
  // into a scroll/zoom, and kill the mobile text-select + tap-flash artifacts
  // that otherwise fire on a long press.
  ...
  el.setAttribute("aria-label", "Portfolio carousel. Use left and right arrow keys to explore projects.");
  ...
  let dragVel = 0; // smoothed CSS px / second, handed to `velocity` on release
  ...
  let lastPointerX = NaN; // last known pointer position, so the follower can
  let lastPointerY = NaN; // re-test its hover state when the row moves
  ...
  let overPanel = false; // the LABEL's state (gated by entry / drag)
  let hoverPanel = false; // raw geometry: is the mouse over a panel right now
  ...
  // Single source of truth for the canvas cursor. Priority: locked (entry /
  // focus) -> grabbing while a drag is live -> grab/pointer over a panel ->
  // default. Keyed off hoverPanel (raw geometry) rather than overPanel, so the
  // label's own gates never take the cursor down with them.
  function updateCursor() {
    if (focusState.active || entryActive || entrySettled) return setCursor("");
    if (dragging) return setCursor("grabbing");
    if (!hoverPanel) return setCursor(""); // off the carousel, plain arrow
    if (INTERACT.drag) return setCursor("grab");
    return setCursor(INTERACT.noClick ? "" : "pointer");
  }

  // Hover changes drive both the cursor and the label. setView applies the
  // label's own gates on top; the cursor reads hoverPanel directly.
  function setHover(on) {
    hoverPanel = on;
    setView(on); // always ends in an updateCursor()
  }
  ...
  // the carousel scrolled beneath a still cursor: a panel arriving under the
  ...
  function setView(on) {
    // never show the follower label during the entry — only in full mode
    if (entryActive || entrySettled) on = false;
    // and never while the row is being pulled — the grabbing hand carries the
    // interaction on its own, the trailing label just fights it
    ...
    // Kill any show/hide still in flight before starting the next one.
    // press -> release -> click queues a fade-in (release, still over the
    // panel) immediately followed by a fade-out (click opens focus). Left to
    // GSAP's default non-overwriting behaviour both survive and the label ends
    // up stuck visible. Scoped so the quickTo x/y trailing tweens are untouched.
  ...
  function onWheel(e) {
    if (inputLocked() || sources.length <= 1) return;
    e.preventDefault();
    pendingFocus = null; // manual scroll cancels a click-to-center in flight
    target += (e.deltaY || e.deltaX) * CONFIG.WHEEL;
    lastInput = performance.now();
    snapped = false; // re-arm the snap on every fresh input
    deferMotion();
  }

  function onPointerDown(e) {
    suppressClick = false;
    if (!INTERACT.drag || inputLocked() || sources.length <= 1) return;
    if (dragging) return; // a second finger never hijacks a live drag
    if (e.button !== 0 && e.pointerType === "mouse") return; // left button only
    // Keep pointer use from leaving the keyboard-focus pause latched after a
    // click. The canvas remains reachable normally with Tab.
    ...
    setView(false); // label out of the way for the pull (also sets the cursor)
    velocity = 0; // grabbing kills any leftover momentum
    target = scroll; // remove the automatic follower's small residual lead
    pendingFocus = null; // and cancels a click-to-center in flight
  ...
  function onPointerMove(e) {
    ...
    if (dragging && e.pointerId === dragPointerId) {
      ...
      // Convert event movement to px / second so flicks behave consistently
      // across browsers and pointer sampling rates.
      const moveAt = performance.now();
      const elapsed = Math.max((moveAt - dragMoveT) / 1000, 1 / 240);
      const instantVelocity = Math.max(-2400, Math.min(2400, (-dx * sens) / elapsed));
      dragVel = dragVel * 0.65 + instantVelocity * 0.35;
      dragMoveT = moveAt;
      lastInput = dragMoveT;
      snapped = false;
      deferMotion();
    ...
    // Hover affordances are mouse-only: a finger has no hover state to
    // describe, so the label would just park wherever you last tapped.
    if (e.pointerType !== "mouse") return;
    // label: eased trail. Keep tracking even while dragging so it's already
    // under the cursor when it fades back in on release.
  ...
  function activatePanelAt(clientX, clientY) {
    if (INTERACT.noClick || inputLocked()) return false;
    deferMotion();
    const hit = panelAtPointer(clientX, clientY);
    if (!hit) return false;
    if (onProjectOpen) {
      onProjectOpen(hit.srcIndex);
      return true;
    }
    // Centered panel -> open right away; anything else glides to center first
    // and the tick loop opens focus once it arrives.
  ...
  function onPointerUp(e) {
    ...
    // Pointer capture can prevent mobile Safari/Chrome from dispatching the
    // synthetic click that normally follows a tap. Activate a clean touch tap
    // here, then consume any click that the browser does happen to emit.
    if (
      e &&
      e.type === "pointerup" &&
      dragPointerType !== "mouse" &&
      !suppressClick &&
      activatePanelAt(e.clientX, e.clientY)
    ) {
      suppressClick = true;
    }
    // let go -> the label fades back in if we're still over a panel
  ...
  function onKeyDown(e) {
    ...
    if ((e.key === "Enter" || e.key === " ") && centeredPanel) {
      e.preventDefault();
      deferMotion();
      openFocus();
      return;
    }
    if (sources.length <= 1 || (e.key !== "ArrowLeft" && e.key !== "ArrowRight")) return;
    e.preventDefault();
    pendingFocus = null;
    velocity = 0;
    automaticVelocity = 0;
    target = centerForIndex(nearestIndex(scroll) + (e.key === "ArrowRight" ? 1 : -1));
    lastInput = performance.now();
    snapped = true;
    deferMotion();
  }

  // Toggle drag / click-to-focus at runtime (the GUI drives this). Keeps the
  // invariant that drag-only mode needs drag on, and that turning drag off
  ...
  function setMotion(next = {}) {
    if (next.enabled !== undefined) motionConfig.enabled = Boolean(next.enabled);
    if (next.direction !== undefined) motionConfig.direction = next.direction === "right" ? "right" : "left";
    if (next.desktopSpeed !== undefined) motionConfig.desktopSpeed = boundedNumber(next.desktopSpeed, motionConfig.desktopSpeed, 10, 60);
    if (next.mobileSpeed !== undefined) motionConfig.mobileSpeed = boundedNumber(next.mobileSpeed, motionConfig.mobileSpeed, 10, 40);
    if (next.resumeDelay !== undefined) motionConfig.resumeDelay = boundedNumber(next.resumeDelay, motionConfig.resumeDelay, 0, 6000);
    if (next.disableForReducedMotion !== undefined) motionConfig.disableForReducedMotion = Boolean(next.disableForReducedMotion);
    deferMotion(0);
  }
  ...
  function openFocus() {
    ...
    setView(false); // also drops the cursor back to default (focus locks input)
    onFocusChange(true);
  }
  ...
  function closeFocus() {
    ...
    const tl = gsap.timeline({
      onComplete: () => {
        ...
        deferMotion();
        updateCursor(); // grab / pointer comes back
      },
    });
    tl.to(
      focusState,
  ...
  // ---- entry animation: rise + scale, controlled overshoot, exact settle ----
  function playEntry() {
    ...
    if (window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
      for (let k = 0; k < pEntry.length; k++) pEntry[k] = 1;
      ...
      onEntryDone(true);
      motionReady = true;
      motionResumeAt = performance.now();
      return;
    }
    ...
    for (let k = 0; k < lastCenterX.length; k++) {
      if (lastCenterX[k] !== undefined) visible.push({ idx: k, x: lastCenterX[k] });
    }
    visible.sort((a, b) => Math.abs(a.x) - Math.abs(b.x));
    const startScale = entryStartScale();
    const overshootProgress = (entryScaleOvershoot() - startScale) / Math.max(1 - startScale, 0.001);
    const tl = gsap.timeline({ delay: ENTRY.delay, defaults: { overwrite: "auto" } });
    let entryEnd = 0;

    visible.forEach(({ idx }, rank) => {
      const at = rank * ENTRY.stagger;
      const settleAt = at + ENTRY.riseDuration;
      entryEnd = Math.max(entryEnd, settleAt + ENTRY.settleDuration);
      tl.to(pEntry, { [idx]: entryPositionOvershoot(), duration: ENTRY.riseDuration, ease: ENTRY.riseEase }, at);
      tl.to(growArr, { [idx]: overshootProgress, duration: ENTRY.riseDuration, ease: ENTRY.riseEase }, at);
      tl.to(pEntry, { [idx]: 1, duration: ENTRY.settleDuration, ease: ENTRY.settleEase }, settleAt);
      tl.to(growArr, { [idx]: 1, duration: ENTRY.settleDuration, ease: ENTRY.settleEase }, settleAt);
    });

    // The glass wakes just after the centered frame and settles with it.
    tl.to(focusState, { lensFx: 1, duration: ENTRY.lensBloom, ease: ENTRY.lensBloomEase }, 0.12);

    // Hand off to the normal full-size carousel at the exact layout values.
    tl.call(
      () => {
        entryActive = false;
        entrySettled = false;
        ...
        motionReady = true;
        motionResumeAt = performance.now();
        automaticVelocity = continuousMotionEnabled() ? automaticSpeed() : 0;
        updateCursor(); // input unlocks -> show the grab hand
      },
      null,
      entryEnd,
    );
  ...
  function onNavigationChange(event) {
    navigationOpen = Boolean(event.detail?.open);
    if (navigationOpen) freezeMotion();
    else deferMotion();
  }
  ...
  function onReducedMotionChange() {
    if (motionConfig.disableForReducedMotion && reducedMotion.matches) freezeMotion();
    else deferMotion();
  }
  function onFocusIn(event) {
    interactiveFocused = mount.contains(event.target);
    freezeMotion();
  }
  function onFocusOut() {
    queueMicrotask(() => {
      interactiveFocused = mount.contains(document.activeElement);
      if (!interactiveFocused) deferMotion();
    });
  }
  window.addEventListener("portfolio-menu-change", onNavigationChange);
  ...
  const viewportObserver = new IntersectionObserver(([entry]) => {
    ...
    if (!carouselVisible) freezeMotion();
    else deferMotion();
  }, { threshold: 0.12 });
  ...
  function tick(frameAt = performance.now()) {
    const now = performance.now();
    const dt = Math.min(0.05, Math.max(0, (frameAt - previousFrameAt) / 1000));
    previousFrameAt = frameAt;

    // A hidden tab gets no WebGL work. The 50ms delta cap above prevents a
    // large jump when the tab becomes visible again.
    if (document.hidden || !carouselVisible) {
      raf = requestAnimationFrame(tick);
      return;
    }

    const canResume = !motionPaused() && now >= motionResumeAt && Math.abs(velocity) < 8;
    if (canResume) {
      // Exponential blending reaches ~99% of target speed in one second.
      const blend = 1 - Math.exp(-4.6 * dt);
      automaticVelocity += (automaticSpeed() - automaticVelocity) * blend;
    } else {
      automaticVelocity = 0;
    }
    ...
    if (!dragging) {
      ...
      if (Math.abs(velocity) < 3) velocity = 0;

      // Constant CSS-pixel speed, independent of display refresh rate.
      target += automaticVelocity * dt;

      // Settle-snap once input has actually stopped (idle time since the last
      // wheel/drag event). Retargets from the CURRENT scroll so it always
      // locks onto whichever panel is truly nearest right now.
      if (
        CONFIG.SNAP &&
    ...
    // physically touching the row.
    // (a click-to-center travel keeps the normal ease — it's a journey across
    // the row, not a settle, and SNAP_EASE would make it crawl)
    const followBase =
      dragging && dragPointerType !== "mouse"
        ...
        : snapped && !pendingFocus
          ? CONFIG.SNAP_EASE
          : CONFIG.EASE;
    const follow = 1 - Math.pow(1 - followBase, dt * 60);
    scroll += (target - scroll) * follow;

    // Keep the running values bounded without changing the visible layout.
    if (!focusState.active && !pendingFocus && totalWidth && Math.abs(scroll) > totalWidth * 1000) {
      const cycles = Math.trunc(scroll / totalWidth);
      const shift = cycles * totalWidth;
      scroll -= shift;
      target -= shift;
      prevScroll -= shift;
    }
    ...
    lensUniforms.uSizeX.value = (LENS.sizeX * DESIGN_STAGE_HEIGHT * responsiveScale()) / Math.max(H, 1);
    lensUniforms.uSizeY.value = (LENS.sizeY * DESIGN_STAGE_HEIGHT * responsiveScale()) / Math.max(H, 1);
  ...
  function onResize() {
    ...
    const activeLoopIndex = nearestIndex(scroll);
    W = mount.clientWidth;
    ...
    lensUniforms.uSamples.value = W <= 1024 ? Math.min(LENS.samples, 10) : LENS.samples;
    recomputeTotal();
    scroll = centerForIndex(activeLoopIndex);
    target = scroll;
  }
  ...
  let destroyed = false;
  document.fonts?.ready.then(() => { if (!destroyed) onResize(); });
  ...
  function destroy() {
    ...
    window.removeEventListener("portfolio-menu-change", onNavigationChange);
  ...
  return {
    ...
    refreshLayout: onResize,
    setInteraction, // toggle drag / click-to-focus at runtime
    setMotion,
