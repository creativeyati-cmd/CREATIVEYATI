...
export const videoSchema = z.object({
  ...
  videoUrl: z.string().url().refine((url) => !!getVideoSource(url), "Use a supported YouTube or Google Drive video URL."),
  orientation: z.enum(["portrait", "landscape"]),
  categoryId: z.union([z.uuid(), z.string().regex(/^[1-9]\d*$/, "Invalid category.")]).nullable().optional(),
...
export const enquirySchema = z.object({
  ...
  message: z.string().trim().min(1, "Tell me a little about your project.").max(5000),
...
export const bachsSettingsSchema = z.object({
  enabled: z.boolean(),
  apiKey: z.union([
    z.literal(""),
    z.string().trim().max(500).refine((value) => value.startsWith("sk_sandbox_") || value.startsWith("sk_live_"), "Use a valid Bachs sandbox or live API key."),
  ]),
