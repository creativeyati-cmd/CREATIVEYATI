...
export default async function AboutPage() {
  ...
  return <main className="about-page public-page">
    ...
    <PublicTextReveal as="article" className="about-note public-note">
      <div className="about-identity">{site.profileImage ? <img className="about-avatar" src={site.profileImage} style={{ objectPosition: focal }} alt="" /> : <span className="about-avatar identity-mark">{initials}</span>}<div><h1 data-reveal>{site.creatorName}</h1><p data-reveal>{site.professionalTitle}</p>{site.availability && <small data-reveal>{site.availability}</small>}</div></div>
