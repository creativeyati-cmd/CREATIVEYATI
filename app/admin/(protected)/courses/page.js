...
export default async function AdminCoursesPage() {
  ...
  return <>
    ...
    {courses.length ? <div className="admin-list course-admin-list">{courses.map((course) => <div key={course.id}>
      ...
      <span className="course-row-actions"><Link href={`/admin/courses/${course.id}/edit`}>Edit</Link><Link href={`/admin/courses/${course.id}/curriculum`}>Curriculum</Link><Link href={`/admin/courses/${course.id}/materials`}>Materials</Link><Link href={`/admin/courses/${course.id}/preview`}>Preview</Link><Link href={`/admin/courses/${course.id}/edit?step=publish`}>Publish</Link></span>
