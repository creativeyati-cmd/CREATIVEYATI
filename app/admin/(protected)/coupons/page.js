...
export default async function CouponsPage({ searchParams }) {
  ...
  return <>
    <div className="admin-title"><p>COMMERCE</p><h1>Coupons</h1><p className="admin-lede">Create controlled discounts for course checkout and monitor redemption limits.</p></div>
    ...
    {error ? <p className="form-error">Coupons could not be loaded.</p> : coupons.length ? <div className="admin-list">{coupons.map((coupon) => <div key={coupon.id}>
    ...
    </div>)}</div> : <p className="empty-state admin-empty-state">No coupons yet.</p>}
  </>;
}
