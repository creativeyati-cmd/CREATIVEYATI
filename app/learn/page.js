...
export default async function LearnPage({ searchParams }) {
  ...
  return <section className="learn-dashboard public-note">
    ...
    {states.length ? <div className="learning-grid">{states.map(({ item, percent, recentLesson, nextLesson }) => <article key={item.id}><Image src={item.course.coverImageUrl} alt={`${item.course.title} cover`} width={640} height={360} sizes="(max-width: 767px) 90vw, 180px" unoptimized /><div><span>{percent}% complete</span><h2>{item.course.title}</h2>{recentLesson && <small>Last viewed: {recentLesson.title}</small>}<progress max="100" value={percent}>{percent}%</progress><Link className="button" href={nextLesson ? `/learn/${item.course.slug}/lesson/${nextLesson.id}` : `/learn/${item.course.slug}`}>{percent === 100 ? "Review course" : "Continue learning"}</Link></div></article>)}</div> : <div className="empty-state"><p>You have not enrolled in a course yet.</p><Link className="button" href="/courses">Browse courses</Link></div>}
