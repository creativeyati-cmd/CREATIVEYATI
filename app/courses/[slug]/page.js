...
export default async function CoursePage({ params }) {
  ...
  return <main className="public-page"><PublicHeader site={site} current="/courses" /><article className="course-detail public-note"><Link className="inline-link" href="/courses">All courses</Link>
    ...
    {course.learningOutcomes.length > 0 && <section><h2>What you will learn</h2><ul>{course.learningOutcomes.map((item) => <li key={item}>{item}</li>)}</ul></section>}
    {course.requirements.length > 0 && <section><h2>Requirements</h2><ul>{course.requirements.map((item) => <li key={item}>{item}</li>)}</ul></section>}
    {course.targetAudience.length > 0 && <section><h2>Who this is for</h2><ul>{course.targetAudience.map((item) => <li key={item}>{item}</li>)}</ul></section>}
    <section className="course-curriculum"><h2>Curriculum</h2>{course.sections.length ? course.sections.map((section) => <div key={section.id}><h3>{section.title}</h3>{section.description && <p className="course-section-description">{section.description}</p>}{section.lessons.map((lesson) => <p key={lesson.id}><span>{lesson.title}</span>{lesson.isPreview ? <Link href={`/courses/${course.slug}/preview/${lesson.id}`}>Preview</Link> : <small>Enrolled students</small>}</p>)}</div>) : <p>Curriculum details will be published soon.</p>}</section>
